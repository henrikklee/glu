...
Artifacts larger than 1 MiB use lazy 1 MiB ranges, while smaller artifacts remain whole requests. Four independently constructed HTTP clients use normal ALPN negotiation rather than forcing a protocol. Each has four request slots and its own priority queue. New attempts are assigned round-robin across those fixed lanes; a free lane slot admits its highest-priority waiter. Fast lanes naturally process more ranges, while a slow lane can hold at most four 1 MiB assignments. This keeps request concurrency independent of active DAG nodes and avoids throughput scoring or a separate health classifier. If references share a digest, their independent random staging files safely converge on the same verified cache entry.
