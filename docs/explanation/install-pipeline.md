...
Artifacts larger than 1 MiB use lazy 1 MiB ranges, while smaller artifacts remain whole requests. Four independently constructed HTTP clients use normal ALPN negotiation rather than forcing a protocol. Each contributes four pool-tagged slots to one central priority queue. Pending ranges are not assigned to a pool; any returned slot immediately serves the highest-priority waiter. Fast pools naturally return and process more slots, while a slow pool can hold at most four 1 MiB assignments. This keeps request concurrency independent of active DAG nodes and avoids throughput scoring, stranded work, or a separate health classifier. If references share a digest, their independent random staging files safely converge on the same verified cache entry.
