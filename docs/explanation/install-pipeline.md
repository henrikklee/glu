...
Each selected package artifact reference remains one DAG download node. The DAG computes static downstream-tail priority; the install-wide downloader uses that priority whenever ordinary request capacity becomes available. When an artifact node starts, it resolves the authenticated GHCR endpoint without following its redirect and immediately reuses the resulting signed CDN URL for that artifact's requests. Active artifact nodes resolve concurrently, so downloads overlap later URL resolutions instead of waiting for the complete closure. A rejected signed URL is refreshed through GHCR. Signed URLs remain in memory and diagnostics retain only their origin.

By default, artifacts larger than 1 MiB are divided into balanced parts targeting 1 MiB, while smaller artifacts remain whole requests. Four independently constructed HTTP clients use normal ALPN negotiation rather than forcing a protocol. Each contributes four pool-tagged slots to one central priority queue. Pending ranges are not assigned to a pool; any returned slot immediately serves the highest-priority waiter. Fast pools naturally return and process more slots, while a slow pool can hold at most four ordinary assignments. This keeps request concurrency independent of active DAG nodes and avoids throughput scoring, stranded work, or a separate health classifier. If references share a digest, their independent random staging files safely converge on the same verified cache entry.
...
Emergency recovery is restricted to pathological work on the highest-priority artifact near the artifact or install tail. A range with no useful progress or a genuinely pathological remaining-time estimate can race a rescue through a fresh HTTP client, temporarily exceeding ordinary admission through a small global allowance. One rescue per affected range can run at a time, with at most two rescues globally. A failed or pathological rescue can be replaced; the first complete attempt wins and the loser is cancelled. Healthy bulk traffic and elapsed duration alone never trigger speculation.

Every request lifecycle and control decision records structured diagnostics: signed-URL resolution, queue admission, selected transport pool and lane load, response origin, headers, negotiated protocol, first body byte, sampled progress, failure chain, retry backoff, emergency evidence, winner, and cancellation. Signed query parameters are never recorded. The normal event timeline also retains `wait_first_byte`, `body_write`, `verify`, and `sync` download subphases. Diagnostics are passive observability and never select transfer behavior.
