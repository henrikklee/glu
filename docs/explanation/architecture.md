...
The install executor and downloader have separate, narrow responsibilities. The DAG keeps one download node per selected package artifact reference and computes that node's static downstream-tail priority. The downloader owns request admission, fixed-size ranges, retry backoff, and the exceptional critical-tail race beneath that node. Ranges and attempts are runtime details; they do not expand the execution plan or weaken the artifact-level digest boundary. Independent references to the same digest use separate random staging files and safely converge on the same verified cache entry.
...
One install-wide downloader gives newly available ordinary request capacity to the highest-priority queued range. Large artifacts use lazy fixed-size ranges rather than a fixed maximum part count, so GCC-sized and multi-gigabyte artifacts keep the same bounded range shape. Request concurrency is transport policy, not the DAG's historical artifact-node lane count.

Ordinary attempts share one logical request budget across four independently constructed HTTP clients. Each client owns a separate connection pool and uses normal ALPN negotiation. This prevents one pathological HTTP/2 connection cohort from coupling the complete install without confusing logical streams with physical connections. Initial work explores every pool. Later ranges choose the pool with the lowest ratio of outstanding bytes to capacity already demonstrated during the install, so a fast pool may accept more streams while a completed slow pool does not automatically receive more work. This selection is a synchronous allocation calculation over passive counters, not a health state machine or background scheduler.

An individual HTTP attempt may fail, but a retryable network outcome does not fail its artifact. Safely committed bytes remain in place and the exact suffix returns to the same priority queue after capped backoff. A small independent emergency allowance exists only for the highest-priority artifact at its final one or two ranges. One pathological tail can race one suffix on a separate HTTP connection pool; the winner cancels the loser. Healthy bulk traffic and elapsed duration alone cannot create speculative requests.
