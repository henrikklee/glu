...
One install-wide downloader gives newly available ordinary request capacity to the highest-priority queued range. Artifacts larger than 1 MiB use lazy 1 MiB ranges, bounding each ordinary assignment while preserving exact coverage for GCC-sized and multi-gigabyte artifacts. Request concurrency is transport policy, not the DAG's historical artifact-node lane count.

Ordinary attempts use four independently constructed HTTP clients with four request slots each. Each client owns a separate connection pool and uses normal ALPN negotiation. New attempts are assigned round-robin across these fixed lanes; when a lane slot becomes free, its priority queue admits that lane's highest-priority waiting range. Faster pools therefore complete and pull more ranges over time, while a slow pool can hold at most four 1 MiB ordinary assignments. No throughput estimate, health classifier, or background allocator selects a pool.
