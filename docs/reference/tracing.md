...
Registry retry history is stored under `diagnostics["startup"]["registry_requests"]`. Each retry identifies the operation, a non-sensitive request ID, attempt number, failure category or HTTP status, accepted `Retry-After`, and selected backoff. URLs, query parameters, headers, and response bodies are excluded. If resolution fails or is interrupted before an execution plan exists, glu writes a failed trace with empty `nodes`, `edges`, and `events` so the retry history is still retained.
