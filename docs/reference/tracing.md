...
| `diagnostics` | object | Structured operation-specific diagnostics keyed by node identity. |
...
Install traces record pre-execution timing under `diagnostics["startup"]`. Host timing separates argument handling, command validation, configuration, operation-lock acquisition, recovery, and total time from CLI main entry to install planning. Client timing separately records the registry request, manifest validation, concurrent installed-state snapshot, remaining plan construction, execution-plan setup, and total command-to-execution time. This also makes time before CLI main entry visible by comparing the harness process duration with the accounted trace duration.

Registry retry history is stored under `diagnostics["startup"]["registry_requests"]`. Each retry identifies the operation, a non-sensitive request ID, attempt number, failure category or HTTP status, accepted `Retry-After`, and selected backoff. URLs, query parameters, headers, and response bodies are excluded. Traces are written only for orchestrator runs. Resolution and manifest-validation failures, including registry interruptions before execution, do not create traces or change `last.json`.

Transfer diagnostics separate logical committed bytes from total wire bytes and record queue time independently from network timing. Their `events` array begins with `source_url_resolved` for the no-follow GHCR redirect, then records central queue admission, the pool ID of the slot eventually assigned, that pool's active load, response origin, response headers, negotiated HTTP version, actual first body byte, sampled body progress, failures, retry backoff, emergency evidence, winner, cancellation, range completion, verification, and artifact completion. A waiting range has no pool identity. Signed CDN query parameters are never recorded. `transport_profile` identifies the allocator/transport configuration that produced the trace. Attempt failure text retains the structured source chain available from reqwest without request URLs. These records are passive: trace persistence and presentation do not participate in scheduling.
...
Fresh `ghcr_bottle_download` nodes record four contiguous summary subphases in the normal event timeline:
...
These artifact-level spans preserve the trace viewer contract. Per-attempt queue, request, body, and transport-pool detail remains in `diagnostics["download:<node-id>"].events` because multipart requests overlap and cannot be represented as one sequential subphase list.
